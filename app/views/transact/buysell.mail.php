...
<?php if($data['type']=="Buy"){?>
<p>As this is a BUY-BTC order you will have to deposit <strong><?=$data['currency']?> <?=round($data['btc_amount']*$data['bid_amount'],0)?>.<?=substr($data['datetime']['time'],6,2)?></strong> as per the instructions below:</p>
...
<p>Please round the amount of your order up to the next higher whole amount, then add the last two digits of the Invoice/Order Number to the amount, as cents. This unique deposit amount is how we can tell it was you who made the deposit.</p>
    <p>Your order total is: <?=$data['currency']?> <?=round($data['btc_amount']*$data['bid_amount'],2)?> Round it up to: <?=$data['currency']?>  <?=round($data['btc_amount']*$data['bid_amount'],0)?></p>
    <p>if your Invoice/Order Number is: RB<?=substr($data['datetime']['time'],4,1)?>00<?=substr($data['datetime']['time'],6,2)?></p>
    <p>Add <?=$data['currency']?>  0.<?=substr($data['datetime']['time'],6,2)?> to the amount... ( the last two digits of your Order Number is <?=substr($data['datetime']['time'],6,2)?> )</p>
    <p>Making the final exact amount to deposit: <?=$data['currency']?> <?=round($data['btc_amount']*$data['bid_amount'],0)?>.<?=substr($data['datetime']['time'],6,2)?> </p>
</div>	
...
<?php }else{
?>
<p>As this is a SELL-BTC order you will have to deposit <?=$data['btc_amount']?> BTC in your address:
<ul>
...
account will be debited for the BTC and credited for equivalent of <strong><?=$data['currency']?> <?=round($data['btc_amount']*$data['bid_amount'],0)?></strong></p>
